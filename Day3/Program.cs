...
namespace Day3
{
    class Program
    {
            ...
            for (int i=0; i<coordsWireOne.Count - 1; i++)
            {
                ...
                for(int j=0; j<coordsWireTwo.Count - 1; j++)
                {
                    ...
                    if (a != null)
                    {
                        ...
                        //intersections.Add(a);
                        if (Math.Abs(a.Item1 + a.Item2) < minDistance)
                        {
                            minDistance = a.Item1 + a.Item2;
                        }
                        if (stepCount < minSteps)
                        {
                            minSteps = stepCount;
                            //Console.WriteLine("Min");
                            Console.WriteLine($"{stepsOne}, {stepsTwo}, {Math.Abs(a.Item1 - coordsWireOne[i].Item1)}, {Math.Abs(a.Item1 - coordsWireTwo[j].Item1)}, {Math.Abs(a.Item2 - coordsWireOne[i].Item2)}, {Math.Abs(a.Item2 - coordsWireTwo[j].Item2)}");
                        }
                    }
                    ...
                    Console.WriteLine($"{stepsOne}, {stepsTwo}");
                }
        ...
        static Tuple<int, int> applyInstruction(string instruction, Tuple<int, int> previousCoords)
        {
            ...
            Tuple<int, int> nextCoords;
            switch(direction)
            {
                case 'U':
                //increase y
                nextCoords = Tuple.Create(previousCoords.Item1, previousCoords.Item2+magnitude);
                return nextCoords;
                case 'D':
                //decrease y
                nextCoords = Tuple.Create(previousCoords.Item1, previousCoords.Item2-magnitude);
                ...
                case 'L':
                //decrease x
                nextCoords = Tuple.Create(previousCoords.Item1-magnitude, previousCoords.Item2);
                return nextCoords;
                case 'R':
                //increase x
                nextCoords = Tuple.Create(previousCoords.Item1+magnitude, previousCoords.Item2);
        ...
        static Tuple<int, int> returnIntersection(Tuple<int, int> A, Tuple<int, int> B, Tuple<int, int> C, Tuple<int, int> D)
        {
            ...
            if (D.Item1 < B.Item1 & D.Item1 < A.Item1 & C.Item1 < B.Item1 & C.Item1 < A.Item1)
            {
                return null;
            }
            if (D.Item1 > B.Item1 & D.Item1 > A.Item1 & C.Item1 > B.Item1 & C.Item1 > A.Item1)
